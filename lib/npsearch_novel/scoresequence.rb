  ...
  class ScoreSequence
    class << self
      ...
      def run(sequence, opt)
        ...
        np_similarity(sequence, opt[:temp_dir], opt[:cdhit_path])
      ...
      def count_c_terminal_glycines(sequence)
        ...
        sequence.potential_cleaved_nps.each do |e|
          if e[:np].match?(/FG$/) && e[:di_clv_end] == 'KR'
            sequence.score += 0.40
          elsif e[:np].match?(/G$/) && e[:di_clv_end] == 'KR'
            sequence.score += 0.25
          elsif e[:np].match?(/G$|GK$|GR$/)
            sequence.score += 0.10
          end
        end
      end
      ...
      def np_similarity(sequence, temp_dir, cdhit_path, results = nil)
        results  = run_cdhit(sequence, temp_dir, cdhit_path) if results.nil?
      ...
      def run_cdhit(sequence, temp_dir, cdhit_path)
